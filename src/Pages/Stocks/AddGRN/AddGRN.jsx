...
function ListGRN() {
  const [customers, setCustomers] = useState([
    { id: 1, Item: 'car', Rate: '5', Quantity: '2',   Type: '%',  Discount: '10', Total: '10000000' },
  ...
  return (
    <div className="bg-white p-6 rounded-md shadow-md w-full ">
          ...
          <tbody>
            {customers.map((customer) => (
              <tr key={customer.id} className="table-row text-gray-700 text-left  bg-[#FEF9FF]">
                ...
                <td className="py-2 px-4 border-b text-[#264164]" data-label="Rate (LKR)">{customer.Rate}</td>
                <td className="py-2 px-4 border-b text-[#264164]" data-label="Quantity">{customer.Quantity}</td>
                <td className="py-2 px-4 border-b text-[#264164]" data-label="Discount Type">{customer.Type}</td>
      ...
      <button className="border border-gray-300 text-gray-500 p-2 rounded-md hover:ring-2 hover:ring-blue-300 hover:ring-opacity-50  text-xs sm:text-xs md:text-sm lg:text-base xl:text-lg 2xl:text-sm xxl:text-sm">+ Add Item to GRN</button>
        ...
        <div className="flex justify-end mt-4 animate-slideInUp">
          <button className="px-6 py-2 bg-purple-500 text-white rounded-md shadow hover:bg-purple-600 transition-all duration-200">+ Add GRN</button>
        </div>
